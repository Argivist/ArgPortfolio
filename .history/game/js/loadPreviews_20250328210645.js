...
function createPreview(row) {
  ...
  let mediaElement;
  if (row.mediaType && row.mediaType.toLowerCase() === 'youtube') {
    // Create an iframe for the YouTube preview.
    // Note: You can add parameters to disable controls if desired.
    mediaElement = document.createElement('img');
    mediaElement.classList.add('preview-video');
    mediaElement.width = "100%";
    mediaElement.height = "100%";
    // Use YouTube's embed URL with minimal parameters for preview.
    mediaElement.src = `https://img.youtube.com/vi/${row.youtubeId}/hqdefault.jpg`;
...
function openModal(row) {
  ...
  if (row.mediaType && row.mediaType.toLowerCase() === 'youtube') {
    ...
    iframe.width = "100%";
    iframe.height = "100%";
    iframe.src = `https://www.youtube.com/embed/${row.youtubeId}?autoplay=1&controls=1&modestbranding=1`;
    iframe.frameBorder = "0";
    iframe.allow = "accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture";
    iframe.allowFullscreen = true;
    iframe.referrerPolicy="strict-
    mediaContainer.appendChild(iframe);
  } else if (row.mediaType && row.mediaType.toLowerCase() === 'video') {
